...
def listLTCOfficeOrder():
    ...
    if (handlerInfo.id == 0):
        list(reversed([j.json() for j in LTCInfo.query.filter(
            LTCInfo.userId == handlerInfo.id and LTCInfo.stageCurrent >= 100).all()]))
...
def listTAOfficeOrder():
    handlerInfo = session.get('userInfo')
    if (handlerInfo.id == 0):
        list(reversed([j.json() for j in TAInfo.query.filter(
            TAInfo.userId == handlerInfo.id and TAInfo.stageCurrent >= 100).all()]))
